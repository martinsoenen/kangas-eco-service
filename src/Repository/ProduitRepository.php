...
class ProduitRepository extends ServiceEntityRepository
{
    ...
    public function findProduitsByCategorie(int $id) {
        ...
        $sql ='
            SELECT p.id, p.sous_categorie_produit_id, p.nom_produit, p.prix_unitaire_ht, p.taux_tva, p.presentation, p.description_detaillee, cp.id, cp.nom, scp.nom
            FROM produit p
    ...
    public function findProduitsBySousCategorie(int $id) {
        $entityManager = $this->getEntityManager()->getConnection();
        $sql ='
            SELECT p.id, p.sous_categorie_produit_id, p.nom_produit, p.prix_unitaire_ht, p.taux_tva, p.presentation, p.description_detaillee, scp.id, scp.nom
            FROM produit p
            INNER JOIN sous_categorie_produit scp ON p.sous_categorie_produit_id = scp.id
            WHERE scp.id = @id
        ';
